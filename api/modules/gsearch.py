...
def get_instagram(name):
    ...
    html = requests.get('https://www.google.com/search?q=' +
                        player + '+nba+instagram&hl=en', headers=google_headers)
    response = str(html.text)
    instagram = re.search(
        'href="https:\/\/www.instagram.com\/([^"]+)"', response)
    if instagram:
...
def get_twitter(name):
    player = name.replace(" ", "+")
    html = requests.get('https://www.google.com/search?q=' +
                        player + '+nba+twitter&hl=en', headers=google_headers)
    response = str(html.text)
...
## STANDALONE FACEBOOK SEARCH ##


def get_facebook(name):
    player = name.replace(" ", "+")
    html = requests.get('https://www.google.com/search?q=' +
                        player + '+nba+facebook&hl=en', headers=google_headers)
    response = str(html.text)
    fb = re.search('facebook.com\/([^"]+)"', response)
    print(fb)
    if fb:
        fb = fb.group(1)
...
def get_all_social(name):
    player = name.replace(" ", "+")
    html = requests.get('https://www.google.com/search?q=' +
                        player + '+nba&hl=en', headers=google_headers)
    response = str(html.text)

    ## REGEX MATCHES FOR SOCIAL MEDIA ##
    twitter = re.search(
        r'<g-link class="fl"><a.*\shref="https:\/\/.*twitter.com\/([^"]+)"', response)
    instagram = re.search(
        r'<g-link class="fl"><a.*\shref="https:\/\/www.instagram.com\/([^"]+)"', response)
    facebook = re.search(
        r'<g-link class="fl"><a.*\shref="https:\/\/www.facebook.com\/([^"]+)"', response)
